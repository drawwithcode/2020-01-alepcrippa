...
function draw() {
  ...
  w=waveS*map(width/2-60,0,height,2500,0); //just empirically found, needed for the gray lines
  ...
  rect(-wave1*3/2,-wave1*3/2,2*wave1*3/2,2*3/2*wave1);
  pop();
  ...
  history.push(wave1);
    ...
    for (let i=0; i < history.length; i++){
      ...
      push();
      rotate(history[i]/50);
      rect(-history[i]*3/2,-history[i]*3/2,2*history[i]*3/2,2*history[i]*3/2);
      pop();
    }
    ...
    history5.push(wave1);
      ...
      for (let i=0; i < history5.length; i++){
        ...
        rect(-history5[i]*3/2,-history5[i]*3/2,2*history5[i]*3/2,2*history5[i]*3/2);
        pop();
      }
    ...
    for (let i=0; i < history2.length; i++){
      ...
      line(0,-2.5*history2[i],0,2.5*history2[i])
      pop();
    }
    ...
    for (let i=0; i < history3.length; i++){
      ...
      line(-2.5*history3[i],0,2.5*history3[i],0)
      pop();
    }
